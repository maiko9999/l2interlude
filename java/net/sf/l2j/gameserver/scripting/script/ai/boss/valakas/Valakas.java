...
public class Valakas extends DefaultNpc
{
	...
	public void onAttacked(Npc npc, Creature attacker, int damage, L2Skill skill)
	{
		...
		if (attacker.getZ() < (npc.getZ() + 200))
		{
			...
			if (npc._i_quest0 == 0)
			{
				final double angle = MathUtil.calculateAngleFrom(npc, attacker);
		...
		if (npc._i_ai2 == 0)
		{
			...
			if ((i1 > 3 && npc._i_ai0 > (npc._i_ai1 * 2)) || i1 > 5)
			{
				...
				final List<Location> plist0 = npc.getPosition().getClosestPositionList(FLEE_POINTS, 3);
				if (!plist0.isEmpty())
				{
					i0 = Rnd.get(3);
					if (i0 == 2)
					...
					npc.getAI().addMoveToDesire(plist0.get(i0), 100000000);
				}
			}
		}
		
		if (npc._i_quest0 == 0 && ((npc._i_ai4 / npc.getStatus().getMaxHp()) * 100) > 1 && npc._i_ai3 > (npc._i_ai4 - npc._i_ai3))
		{
			npc._i_ai3 = 0;
			npc._i_ai4 = 0;
