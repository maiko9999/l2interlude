...
public class RaidPrivateStandard extends MonsterAI
{
	...
	public void onPartyAttacked(Npc caller, Npc called, Creature target, int damage)
	{
		...
		if (caller == called)
		{
			...
			i0 = 10;
			if (!Config.RAID_DISABLE_CURSE && target.getStatus().getLevel() > (called.getStatus().getLevel() + 8) && getAbnormalLevel(target, raidCurse) == -1)
			{
	...
	public void onSeeSpell(Npc npc, Player caster, L2Skill skill, Creature[] targets, boolean isPet)
	{
		...
		if (!Config.RAID_DISABLE_CURSE && caster.getStatus().getLevel() > (npc.getStatus().getLevel() + 8) && getAbnormalLevel(caster, raidMute) <= 0)
		{
