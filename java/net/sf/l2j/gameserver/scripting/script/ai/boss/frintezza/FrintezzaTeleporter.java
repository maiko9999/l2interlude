...
public class FrintezzaTeleporter extends DefaultNpc
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.equalsIgnoreCase("enter"))
		{
			...
			else if (Config.NEED_ITEM_FRINTEZZA && !player.getInventory().hasItems(8073))
				htmltext = "frintessa_teleporter004.htm";
			else
			{
				final int totalMembers = commandChannel.getMembers().size();
				
				if (totalMembers < Config.FRINTEZZA_MINIMUM_ALLOWED_PLAYERS)
				{
				    NpcHtmlMessage html = new NpcHtmlMessage(npc.getObjectId());
				    html.setFile(player.getLocale(), "html/script/ai/boss/frintezza/FrintezzaTeleporter/frintessa_teleporter_min_members.htm");
				    html.replace("%maxMembers%", String.valueOf(Config.FRINTEZZA_MINIMUM_ALLOWED_PLAYERS));
				    player.sendPacket(html);
				}
				
				if (totalMembers > Config.FRINTEZZA_MAXIMUM_ALLOWED_PLAYERS)
				{
				    NpcHtmlMessage html = new NpcHtmlMessage(npc.getObjectId());
				    html.setFile(player.getLocale(), "html/script/ai/boss/frintezza/FrintezzaTeleporter/frintessa_teleporter_max_members.htm");
				    html.replace("%maxMembers%", String.valueOf(Config.FRINTEZZA_MAXIMUM_ALLOWED_PLAYERS));
				    player.sendPacket(html);
				}
				
				final int ccSize = commandChannel.getParties().size();
				if (ccSize < Config.FRINTEZZA_MINIMUM_PARTIES)
					htmltext = "frintessa_teleporter005.htm";
				else if (ccSize > Config.FRINTEZZA_MAXIMUM_PARTIES)
