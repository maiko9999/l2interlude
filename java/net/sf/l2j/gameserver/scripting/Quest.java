...
public class Quest
{
	...
	private void showResult(Npc npc, Creature creature, String result)
	{
		...
		if (result.endsWith(".htm") || result.endsWith(".html"))
		{
			...
			if (result.startsWith("html/"))
				html.setFile(player.getLocale(), result);
			else if (isRealQuest())
				html.setFile(player.getLocale(), "html/script/quest/" + getName() + "/" + result);
			else
				html.setFile(player.getLocale(), "html/script/" + getDescr() + "/" + getName() + "/" + result);
	...
	{
		if (isRealQuest())
			return HTMLData.getInstance().getHtm(player.getLocale(), "html/script/quest/" + getName() + "/" + fileName);
		
		return HTMLData.getInstance().getHtm(player.getLocale(), "html/script/" + getDescr() + "/" + getName() + "/" + fileName);
	}
	...
	public void ssqEventGiveItem(Npc npc, Creature killer, int count)
	{
		...
		{
			// Non wiz class gets 30% bonus when < 80.
			if (dist < 80)
				num += num * 0.3;
			
			// Other: 15% bonus or 7% penalty.
			if (Rnd.nextBoolean())
				num += num * 0.15;
			else
				num -= num * 0.07;
