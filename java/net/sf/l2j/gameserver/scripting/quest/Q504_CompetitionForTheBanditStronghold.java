...
public class Q504_CompetitionForTheBanditStronghold extends Quest
{
	...
	public String onTalk(Npc npc, Player player)
	{
		...
		if (!BANDIT_STRONGHOLD.isWaitingBattle())
			htmltext = getHtmlText(player, "35437-09.htm").replace("%nextSiege%", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(BANDIT_STRONGHOLD.getSiegeDate().getTime()));
		...
		else
		{
			switch (st.getState())
			{
				case CREATED:
					if (!BANDIT_STRONGHOLD.isWaitingBattle())
						htmltext = getHtmlText(player, "35437-03.htm").replace("%nextSiege%", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(BANDIT_STRONGHOLD.getSiegeDate().getTime()));
