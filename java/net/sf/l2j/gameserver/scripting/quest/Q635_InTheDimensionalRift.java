...
import net.sf.l2j.gameserver.scripting.script.teleport.OracleTeleporter;
...
public class Q635_InTheDimensionalRift extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		if (event.equalsIgnoreCase("dimension_keeper_07.htm"))
		{
			...
			player.getMissions().update(MissionType.ENTER_RIFT);
		...
		else if (event.equalsIgnoreCase("rift_watcher_05.htm"))
		{
			...
			if (player != c0)
			{
				String html = getHtmlText(player, "rift_watcher_02.htm");
			...
			for (Player c1 : party0.getMembers())
			{
				...
				{
					String html = getHtmlText(player, "rift_watcher_03a.htm");
				...
				if (c1.getInventory().getItemCount(DIMENSIONAL_FRAGMENT) < stonesToRemove)
				{
					...
					{
						String html = getHtmlText(player, "rift_watcher_03.htm");
						html = html.replace("<?name?>", c1.getName());
						return html;
					}
				}
			...
			if (rlist0 != null)
			{
				...
				if (i3 != -2 && i5 >= 3)
				{
					...
					for (int i4 = 0; i4 < (Config.ENTER_ANAKAZEL ? 9 : 8); i4++)
					{
						if (i3 == -1)
						{
							i5 = Rnd.get(Config.ENTER_ANAKAZEL ? 9 : 8);
							room0 = rlist0.get(i5);
		...
		else if (event.equalsIgnoreCase("rift_watcher_06.htm"))
		{
			...
			if (i0 == 0)
			{
				...
				if (id >= 0)
				{
					player.teleportTo(OracleTeleporter.RETURN_LOCATIONS.get(id), 0);
					
					if (ArraysUtil.contains(OracleTeleporter.TEMPLE_PRIEST, npc.getNpcId()))
		...
		else if (event.equalsIgnoreCase("ChangeRiftRoom"))
		{
			...
			final List<RoomInfo> rlist1 = rlist0.stream().filter(r -> !r.isLocked() && (Config.ENTER_ANAKAZEL ? true : r.getIndex() != 8)).toList();
	...
	public String onTimer(String name, Npc npc, Player player)
	{
		...
		if (name.equalsIgnoreCase("2001"))
		{
			if (room0 != null)
			{
				List<Integer> memberIds = room0.getMemberIds();
