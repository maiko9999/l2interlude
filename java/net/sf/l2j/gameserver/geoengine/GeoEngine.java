...
public class GeoEngine
{
	...
	/**
	 * Simple raycast to find the first collision point from origin to target.<br>
	 * Note: Ignores geodata modification of given {@link IGeoObject}.
	 * @param ox : Origin X coordinate.
	 * @param oy : Origin Y coordinate.
	 * @param oz : Origin Z coordinate.
	 ...
	 * @param tz : Target Z coordinate.
	 * @param ignore : The {@link IGeoObject}, which geodata modification is ignored and original geodata picked instead.
	 * @return The coordinates of the first collision point as an int array {x, y, z}. Returns null if no collision.
	 */
	public final Location raycast(int ox, int oy, int oz, int tx, int ty, int tz, IGeoObject ignore)
	{
		// Check origin coordinates.
		if (World.isOutOfWorld(ox, oy))
			return null;
		...
		if (gox == gtx && goy == gty)
			return index == block.getIndexBelow(gtx, gty, tz + GeoStructure.CELL_HEIGHT, ignore) ? null : new Location(tx, ty, tz);
		...
		while (gox != gtx || goy != gty)
		{
			...
			// Get line of sight height (including Z slope).
			double losz = oz + Config.MAX_OBSTACLE_HEIGHT;
			losz += mz * Math.sqrt((checkX - ox) * (checkX - ox) + (checkY - oy) * (checkY - oy));
			...
			// Next cell's does not exist (no geodata with valid condition), return collision point.
			if (index < 0)
			{
				double distance = Math.sqrt((checkX - ox) * (checkX - ox) + (checkY - oy) * (checkY - oy));
				int collisionZ = (int) (oz + mz * distance);
				return new Location(checkX, checkY, collisionZ);
			}
			
			// Get next cell's layer height.
			int z = block.getHeight(index, ignore);
			
			// Perform line of sight check (next cell is above line of sight line), return collision point.
			if (z > losz)
			{
				double distance = Math.sqrt((checkX - ox) * (checkX - ox) + (checkY - oy) * (checkY - oy));
				int collisionZ = (int) (oz + mz * distance);
			...
			// Next cell is accessible, update last valid coordinates and NSWE.
			lastValidX = checkX;
			lastValidY = checkY;
			lastValidZ = groundZ;
			groundZ = z;
	...
	/**
	 * Check movement from coordinates to coordinates, but treat not fully passable cells as obstacles.<br>
	 * Note: The Z coordinates are supposed to be already validated geodata coordinates.
	 * @param ox : Origin X coordinate.
	 * @param oy : Origin Y coordinate.
	 * @param oz : Origin Z coordinate.
	 ...
	 * @return True, when target coordinates are reachable from origin coordinates.
	 */
	public final boolean canMoveClear(int ox, int oy, int oz, int tx, int ty, int tz, ExServerPrimitive debug)
	{
		// Check target coordinates.
		if (World.isOutOfWorld(tx, ty))
			return false;
		...
		// Check movement within the same cell.
		if (gox == gtx && goy == gty)
			return goz == getHeight(tx, ty, tz);
		...
		while (gox != gtx || goy != gty)
		{
			...
			// Check if the current cell is not passable in all four directions.
	 ...
	 * @param tz : target z
	 * @param playable : moving object is playable?
	 * @param nearestZLimit : limit for finding the nearest Z height;
	 * @param debug : The debug packet to add debug informations in.
	 * @return {@code List<Location>} : complete path from nodes
	 */
	public List<Location> findPath(int ox, int oy, int oz, int tx, int ty, int tz, boolean playable, int nearestZLimit, ExServerPrimitive debug)
	{
		// Check target coordinates.
		if (World.isOutOfWorld(tx, ty))
		...
		final int gtz = getHeightNearest(gtx, gty, tz);
		if (Math.abs(gtz - tz) > nearestZLimit)
			return Collections.emptyList();
		...
		while (point.hasNext())
		{
			...
			if (canMoveClear(nodeAx, nodeAy, nodeAz, nodeC.getX(), nodeC.getY(), nodeC.getZ(), null))
			{
