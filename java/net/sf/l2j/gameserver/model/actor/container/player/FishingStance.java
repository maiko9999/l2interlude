...
public class FishingStance
{
		...
		switch (group)
		{
			...
			case 1: // normal fish
				switch (_lureId)
				{
					...
					case 6519, 8505, 6520, 6521, 8507: // all theese lures (green) are prefered by fast-moving (nimble) fish (type 1)
						if (check <= 54)
							type = 1;
						else if (check <= 74)
							type = 0;
					...
					case 6522, 8508, 6523, 6524, 8510: // all theese lures (purple) are prefered by fat fish (type 0)
						if (check <= 54)
							type = 0;
						else if (check <= 74)
							type = 1;
					...
					case 6525, 8511, 6526, 6527, 8513: // all theese lures (yellow) are prefered by ugly fish (type 2)
						if (check <= 55)
							type = 2;
						else if (check <= 74)
							type = 1;
	...
	public void start(Location baitLoc, ItemInstance lure)
	{
		...
		switch (_lureId)
		{
			...
			case 8485, 8506, 8509, 8512: // green/purple/yellow/prize-winning luminous.
				group = 2;
				break;
		}
		...
		if (_lookingForFish == null)
		{
			...
			else if (_lureId == 6520 || _lureId == 6523 || _lureId == 6526 || (_lureId >= 8505 && _lureId <= 8513) || (_lureId >= 7610 && _lureId <= 7613) || (_lureId >= 7807 && _lureId <= 7809) || (_lureId >= 8484 && _lureId <= 8486) || _lureId == 8548) // medium grade, beginner, prize-winning & quest special bait
