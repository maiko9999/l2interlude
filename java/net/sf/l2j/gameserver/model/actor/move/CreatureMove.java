...
{
	protected static final CLogger LOGGER = new CLogger(CreatureMove.class.getName());
	...
	public boolean maybeStartOffensiveFollow(Creature target, int weaponAttackRange)
	{
		...
		if (!shouldFollow)
		{
			if (_actor instanceof Npc && !GeoEngine.getInstance().canSeeTarget(_actor, target) && !_actor.isMovementDisabled() && _actor.getAI().getCurrentIntention().canMoveToTarget())
				startOffensiveFollow(target, (int) target.getCollisionRadius());
			else
				return false;
		}
		else if (!_actor.isMovementDisabled() && _actor.getAI().getCurrentIntention().canMoveToTarget())
