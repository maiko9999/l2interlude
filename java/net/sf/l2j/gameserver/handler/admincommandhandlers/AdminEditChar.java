...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	public void useAdminCommand(String command, Player player)
	{
		...
		else if (command.startsWith("admin_party_info"))
		{
			...
			html.setFile(player.getLocale(), "html/admin/partyinfo.htm");
		...
		else if (command.startsWith("admin_reset"))
		{
			if (!st.hasMoreTokens())
			{
				player.sendMessage("Usage: //reset <clan_graduate>");
				return;
			}
			...
			switch (st.nextToken())
			{
				case "clan_graduate":
					...
					if (clan != null)
					{
						...
						player.sendMessage("All Clan graduates were cleaned.");
					}
					break;
			...
			switch (st.nextToken())
			{
				...
				case "color":
					try
					{
						...
						targetPlayer.getAppearance().setNameColor(colorName);
						targetPlayer.setNameColor(colorName);
						targetPlayer.broadcastUserInfo();
				...
				case "tcolor":
					try
					{
						...
						targetPlayer.getAppearance().setTitleColor(colorTitle);
						targetPlayer.setTitleColor(colorTitle);
						targetPlayer.broadcastUserInfo();
	...
	public static void gatherPlayerInfo(Player player, Player targetPlayer, NpcHtmlMessage html)
	{
		...
		html.setFile(player.getLocale(), "html/admin/charinfo.htm");
		html.replace("%name%", targetPlayer.getName());
		...
		html.replace("%ip%", (targetPlayer.getClient() == null || targetPlayer.getClient().isDetached()) ? "Offline trader" : Config.PROXY ? targetPlayer.getClient().getRealIpAddress() : targetPlayer.getClient().getConnection().getInetAddress().getHostAddress());
	}
