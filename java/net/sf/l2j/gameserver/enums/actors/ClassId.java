...
public enum ClassId
{
	...
	private final void createSubclasses()
	{
		...
		for (ClassId classId : VALUES)
		{
			...
			if (!Config.CLASS_OVERLORD && (classId == OVERLORD || classId == WARSMITH))
				continue;
			...
			if (!Config.RACE_ELF && (_race == ClassRace.ELF && classId._race == ClassRace.DARK_ELF) || (_race == ClassRace.DARK_ELF && classId._race == ClassRace.ELF))
				continue;
			
			_subclasses.add(classId);
		}
		
		if (!Config.RESTRICTED_CLASSES)
		{
